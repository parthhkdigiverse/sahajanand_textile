...
import { Receipt, IndianRupee, FileText, Printer, Download, Send, Plus } from "lucide-react";

export const Route = createFileRoute("/gst-billing")({
  head: () => ({ meta: [{ title: "GST Billing — Vastra ERP" }] }),
...
function GstBillingPage() {
  ...
  return (
    <ModuleShell title="GST Billing" subtitle="Create GST-compliant tax invoices with CGST/SGST/IGST."
      actions={<>
        <Btn variant="outline"><Download className="h-4 w-4" /> PDF</Btn>
        <Btn variant="outline"><Printer className="h-4 w-4" /> Print</Btn>
        <Btn variant="brand"><Send className="h-4 w-4" /> Save & Send</Btn>
      </>}>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <StatCard label="GST Output" value={inrCompact(INVOICES.reduce((s, i) => s + i.gst, 0))} icon={FileText} tone="info" />
        <StatCard label="Overdue" value={INVOICES.filter(i => i.status === "overdue").length} tone="danger" />
      </div>

      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-3">
        <SectionCard className="xl:col-span-2" title="Tax Invoice" subtitle="INV/26-27/2208 · Draft">
          <div className="rounded-xl bg-surface p-5 text-[13px]">
            <div className="flex items-start justify-between border-b border-border pb-4">
              ...
              <div className="text-right">
                <div className="rounded-md bg-brand/10 px-3 py-1 text-brand">
                  <div className="text-[10px] font-semibold uppercase tracking-wider">Tax Invoice</div>
                  <div className="font-mono text-[14px]">INV/26-27/2208</div>
                </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="mt-1 font-semibold">Krishna Silk House</div>
              ...
              <div>
                <div className="text-[10.5px] font-semibold uppercase tracking-wider text-muted-foreground">Transport</div>
                <div className="mt-1 text-[12px]">VRL Logistics · GJ-05-AB-4421</div>
                <div className="text-[11px] text-muted-foreground">E-Way EWB-2609-871-4412 · 6 km</div>
                <div className="text-[11px] text-muted-foreground">Terms: 45 days · Due: {shortDate(new Date(Date.now() + 45 * 864e5))}</div>
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-6">
              ...
              <div className="ml-auto w-full max-w-xs space-y-1 font-mono text-[12.5px]">
                <TotRow label="Taxable Value" value={inr(sub)} />
        ...
        <div className="space-y-4">
          <SectionCard title="Invoice Settings">
            <div className="space-y-2 text-[13px]">
              <Field label="Invoice Type" value="Tax Invoice (B2B)" />
              <Field label="Payment Terms" value="45 Days" />
          ...
          <SectionCard title="Amount Summary">
            <ul className="space-y-1.5 text-[13px]">
              <li className="flex justify-between"><span className="text-muted-foreground">Taxable</span><span className="font-mono">{inr(sub)}</span></li>
              <li className="flex justify-between"><span className="text-muted-foreground">CGST @ mix</span><span className="font-mono">{inr(cgst)}</span></li>
              <li className="flex justify-between"><span className="text-muted-foreground">SGST @ mix</span><span className="font-mono">{inr(sgst)}</span></li>
              <li className="mt-1 flex justify-between border-t border-border pt-2 text-[15px] font-semibold"><span>Total</span><span className="font-mono text-brand">{inr(total)}</span></li>
            </ul>
          </SectionCard>
        </div>
      </div>
