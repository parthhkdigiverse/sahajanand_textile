...
export const Route = createFileRoute("/delivery-challan")({
  ...
  component: DeliveryChallanRouteComponent,
});

function DeliveryChallanRouteComponent() {
  const matchDetail = useMatch({ from: "/delivery-challan/$id", shouldThrow: false });
  if (matchDetail) {
    return <Outlet />;
  }
  return <DeliveryChallanListPage />;
}

function DeliveryChallanListPage() {
  const cols: Column<(typeof CHALLANS)[number]>[] = [
    {
      ...
      render: (r) => (
        <Link
          to="/delivery-challan/$id"
          params={{ id: r.id }}
          className="font-mono text-brand hover:underline font-semibold"
        >
          {r.id}
        </Link>
      ),
    ...
    },
    {
      ...
      render: (r) => (
        <div>
          <div>{r.transport}</div>
          <div className="font-mono text-[11px] text-muted-foreground">{r.vehicle}</div>
        </div>
      ),
    },
  ...
  return (
    <ModuleShell
      ...
      subtitle="All outward challans with transport & GPS status."
      actions={
        <Btn variant="brand">
    ...
    >
      ...
      <div className="mt-4">
        <DataTable columns={cols} rows={CHALLANS} addLabel="New Challan" onAdd={() => {}} />
      </div>
    </ModuleShell>
  );
}